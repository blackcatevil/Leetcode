...
int** levelOrderBottom(struct TreeNode* root, int** columnSizes, int* returnSize)
{
  ...
  *columnSizes = NULL;
  ...
  for (int i = 0; i < size; i++) {
    ans[size-1-i] = malloc(sizeof(int)*colsize[i]);
    for (int j = 0; j < colsize[i]; j++)
      ans[size-1-i][j] = nodes[i][j]->val;
    free(nodes[i]);
  }
  free(nodes);
  
  *returnSize = size;
  // Reverse columnSize array for judgement.
  for (int i = 0; i < size/2; i++) {
    int tmp = colsize[i];
    colsize[i] = colsize[size-1-i];
    colsize[size-1-i] = tmp;
  }
  *columnSizes = colsize;
